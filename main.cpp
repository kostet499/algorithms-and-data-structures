...
void z_function(const string &pattern, vector<unsigned int> &z_values) {
...
void prefix_func(const string &pattern, vector<unsigned int> &prefix_values) {
...
vector<size_t > z_to_p(const vector<size_t > &z_values) {
    vector<size_t > prefix_values(z_values.size(), 0);
    ...
    for(size_t i = prefix_values.size() - 1; i > - 1; --i) {
...
vector<size_t > p_to_z(const vector<size_t > &prefix_values) {
    vector<size_t> z_values(prefix_values.size(), 0);
...
int local_prefix_func(const string &pattern, const vector<size_t > &prefix_values, char c, int last_prefix) {
